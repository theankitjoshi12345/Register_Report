...
export function Amount({ name, label, value, signed, onChange, errors }: {
  name: string; label: string; value: string; signed?: boolean; onChange: (value: string) => void; errors: FieldErrors
}) {
  return (
    <div>
      ...
      <div className="relative">
        ...
        <input id={name} name={name} required type="number" step="0.01" min={signed ? undefined : 0}
...
export function ScratchFields({ form, catalog, errors, onChange }: {
  ...
  return (
    <section>
      ...
      <p className="mb-3 text-sm text-slate-600">020 means ticket 020 was the last ticket sold; the remaining tickets start at 021. Leave the number empty when the roll sold out. Enter 0 new rolls when none were added.</p>
            ...
            return (
              <tr className="border-t border-slate-100" key={item.slot_number}>
                ...
                <td className="px-4 py-3">
                  <input name={endingName} aria-label={`Slot ${item.slot_number} ending number`} type="number" min="0" max={slot?.max_ticket_number} step="1"
                    ...
                    onChange={(event) => updateRow({ ending_number: event.target.value })} className={inputClass} />
